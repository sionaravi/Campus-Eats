...
const CounselorsCafe = ({navigation}) => {
  ...
  useEffect(() => {
    ...
    const docRef = favoritesRef.doc('CounselorsCafe');
    docRef.get().then((doc) => {
      if (doc.exists) {
  ...
  const toggleFavorite = async () => {
    ...
    const docRef = firebase.firestore().collection('Favorites').doc(user.uid).collection('Restaurants').doc('CounselorsCafe');
